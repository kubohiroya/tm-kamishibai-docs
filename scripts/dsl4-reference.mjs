...
export function renderReferenceDocument({schema, annotations, lock}) {
  ...
  const sections = annotations.sections
    ...
    .join('\n\n');
  const schemaUrl = lock.schemaUrl;
  const sourceCommitUrl = `https://github.com/${lock.repository}/commit/${lock.commit}`;
...
Schema固定commit: [\`${lock.commit.slice(0, 7)}\`](${sourceCommitUrl})\\
...
この文書は、固定snapshotの[DSL 4.0 JSON Schema](${schemaUrl})とCC BY-SA 4.0の日本語Annotationから
