...
> 正式な`v4.0.0`ではありません。この文書の説明はrc.5固定実装を基準とし、将来版で同じ画面や
> ビルド結果になることは保証しません。
...
<p class="application-callout"><strong>利用前に確認:</strong> 4.0.0-rc.5は公開プレリリースです。安定版4.0.0ではないため、利用する版で台本の検査、ブラウザーでの確認、完成ファイルの作成が使えるかを、制作環境の担当者が確認します。</p>
