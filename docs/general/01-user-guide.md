...
台本作者は`setLoadingCostume=loading1,loading2,loading3`のように画像アセット名を指定できます。複数指定されている場合は、通常アセットの読込番号に応じて画像が順番に切り替わります。指定がない台本では、アプリ組み込みのLoadingコスチュームが表示されます。
