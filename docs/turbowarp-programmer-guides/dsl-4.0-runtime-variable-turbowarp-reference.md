...
現行公開仕様の基準: tmpose-kamishibai 4.0.0-rc.5（`f323a54`）、2026年8月15日

文書状態: **受け入れ済み・実装済み利用契約（既定OFF）**。[実装Issue #597](https://github.com/kubohiroya/tmpose-kamishibai/issues/597)で
source実装とtestを追加しています。3つのfeature flagは既定OFFのため、追加surfaceは4.0.0-rc.5の現行公開APIには含まれません。\
...
4.0.0-rc.5でTurboWarpの通常の変数blockから参照できるランタイム所有のStage変数は、次の2つだけです。
...
台本トップレベルの`variables:`はこの2変数とは別で、rc.5にはTurboWarpから直接参照する公開blockがありません。
...
| extension version                     | rc.5に非表示reporterが存在                                 | 公開推奨                                   |
| last error                            | rc.5に生のmessageを返す非表示reporterが存在                | 生messageは非公開                          |
...
rc.5の非表示`statusReporter`はapplication shellの状態を返し、controllerの物語`status`とは意味が異なります。
...
4.0.0-rc.5のcontrollerは、`branch` actionを開始したときのトップレベル`variables:`を不変snapshotにして
...
したがって、rc.5では「台本`variables:`は式で参照できるが、TurboWarp blockからは直接参照できない」という非対称が
...
- [`runtime-controller.js`](https://github.com/kubohiroya/tmpose-kamishibai/blob/f323a5475d4c6240a255f8a6f5b6c5d68b9ea7b6/src/dsl4/runtime-controller.js): snapshot、台本変数、generation、診断、event
- [`pose-feedback-policy.js`](https://github.com/kubohiroya/tmpose-kamishibai/blob/f323a5475d4c6240a255f8a6f5b6c5d68b9ea7b6/src/dsl4/pose-feedback-policy.js): pose state eventの6 field
- [`navigation-session.js`](https://github.com/kubohiroya/tmpose-kamishibai/blob/f323a5475d4c6240a255f8a6f5b6c5d68b9ea7b6/src/dsl4/navigation-session.js): navigation snapshot
- [`dsl4-runtime-extension-entry.js`](https://github.com/kubohiroya/tmpose-kamishibai/blob/f323a5475d4c6240a255f8a6f5b6c5d68b9ea7b6/scripts/sb3/dsl4-runtime-extension-entry.js): rc.5の非表示reporter
- [`project.source.json`](https://github.com/kubohiroya/tmpose-kamishibai/blob/f323a5475d4c6240a255f8a6f5b6c5d68b9ea7b6/release-sources/4.0.0-rc.5/app/project.source.json): 現行SB3のStage変数
