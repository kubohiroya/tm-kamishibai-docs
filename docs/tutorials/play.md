...
標準実行環境: `4.0.0-rc.5`（公開プレリリース）\
このチュートリアルの公開サンプル: `4.0.0-rc.3`（既存成果物）\
...
「Web版を開く」を押します。作品の説明で、この公開サンプルの対象バージョンが`4.0.0-rc.3`であることと、カメラを
使うことを確認してから開きます。
...
表示されます。これは公開サンプル自身の版表示です。`Version 4.0.0-rc.3`を確認し、右上の「閉じる」を押すと
作品が始まります。Standard Runtimeの最新公開プレリリースrc.5とは版が異なります。
