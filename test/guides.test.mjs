...
const extensionGuide = readFileSync(
  new URL('../docs/developer-guides/08-extension-guide.md', import.meta.url),
  'utf8',
);
const applicationGuide = readFileSync(
  new URL('../docs/user-guides/09-application-materials-guide.md', import.meta.url),
  'utf8',
);
...
test('keeps the extension guide as an index plus fifteen extension pages', () => {
  ...
  assert.equal(findDocument('08-extension-guide.md')?.expectedPdfPageCount, 16);
...
test('keeps the application guide in the requested eight-page allocation', () => {
  ...
  assert.equal(findDocument('09-application-materials-guide.md')?.expectedPdfPageCount, 8);
