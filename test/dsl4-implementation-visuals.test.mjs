...
test('labels the captures as implementation evidence instead of a formal release', () => {
  ...
  assert.match(implementationWalkthrough, /正式公開プレイヤーの操作説明ではありません/u);
  assert.match(implementationWalkthrough, /GitHub Releaseとnpmの`latest`/u);
});

test('keeps implementation evidence out of the adult overview and in the developer walkthrough', () => {
  assert.doesNotMatch(adultOverview, /^## 実装スナップショット$/mu);
  assert.doesNotMatch(adultOverview, /dsl4-implementation-[^"\s]+\.jpg/u);
  for (const capture of captures) {
    assert.match(
  ...
  assert.equal(
    (implementationWalkthrough.match(/dsl4-implementation-[^\s)]+\.jpg/gu) ?? []).length,
    2,
  );
  assert.doesNotMatch(implementationWalkthrough, /dsl4-implementation-scene\.jpg/u);
  assert.equal(
    existsSync(new URL('../docs/images/dsl4-implementation-scene.jpg', import.meta.url)),
    false,
  );
  assert.doesNotMatch(theme, /\.dsl4-snapshot-gallery/u);
});

test('publishes the implementation call map', () => {
