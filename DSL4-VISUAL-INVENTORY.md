...
採用しません。正式UIの操作画像は#34、#41、#47のcapture台帳で管理します。
...
| `executive-summary-adult-4.0`「仕組みを詳しく知る」以降                                                                                      | projectが何を経て上演されるか                              | 「プロジェクトから上演まで」の既存図がある                             | 既存図で十分                                               | P1     | semantic HTML、caption、失敗時の本文注記                    | #88で表示確認済み                              |
...
| [`internal-specification-4.0`](docs/developer-guides/internal-specification-4.0.md)「権威関係とアーキテクチャ」                              | sourceからadapterまでの依存方向は何か                      | 7段の既存図がある                                                      | 既存図で十分                                               | P1     | semantic HTML、caption、診断surface注記                     | #88で表示確認済み                              |
