...
短い直線的な流れは、読み上げ順を保てるsemantic HTMLと`.concept-flow`を正本にします。複数状態、
lifeline、commit／rollbackを含む実装図は、JavaScriptなしでWebと印刷に使え、`title`、`desc`、本文の
同値説明を持つSVGを使用します。正式UIの操作画像を追加する場合は#34と#47のcapture台帳を参照します。

#41の操作説明書は、公開URL、画面の案内、操作順を文章・表で完結させます。版、checksum、実機確認などの
保守情報は機械可読manifestへ分離します。rc.5固定commitの再現確認を目的とする実装スナップショットと
実装解析図は#157で別管理し、一般利用者向けの正式操作画面とは表記・provenanceを分けます。
...
| [`dsl-4.0-runtime-block-reference`](docs/turbowarp-programmer-guides/dsl-4.0-runtime-block-reference.md)7 member章             | 実パレットの区切り、順序、表示対象をどこで確認するか | **rc.5実画面を追加**。memberセパレータ単位の7 JPEGを各章冒頭へ配置             | P3     | 固定SB3、意味のあるalt、直後の表を検索用正本とする |
...
| [`internal-specification-4.0`](docs/developer-guides/internal-specification-4.0.md)全体                                        | sourceからadapterまでの依存、状態、transactionは何か | **rc.5実装図を追加**。architecture、build／runtime／reload sequence、2状態遷移 | P1／P2 | SVG 7図、`title`・`desc`・alt・本文同値説明        |
...
- 既存のP1図は、利用者の体験、安全停止、作者の制作順、3.2からの変換を覆っている。
- 内部仕様書はrc.5実装に基づくarchitecture、source build、RuntimeStatus、通常実行、live reload状態／sequence、
  asset transactionの7 SVGで責任分界と安全停止を補強する。
...
2026年8月15日の#157では、公開rc.5の実動作と、DSL 3.2資料と同程度の実装図を求める要望を再評価しました。
一般向け概要へは実装確認画面を重複掲載せず、開発者向け追試にvalid、runtime title、invalidの3画面を置きます。
内部構造は内部仕様書の7 SVGを正本とし、source fileとtestへの追跡表を併記します。samples repositoryは
撮影・作図の入力に使用しません。
